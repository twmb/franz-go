...
// topic ID different from the old incarnation's: a client starting in the
// gap would briefly consume the dying incarnation.
...
func TestTopicRecreation(t *testing.T) {
	...
	// Fewer records than the consumed position: detection converges at
	// every version (worst case via an out of range reset).
