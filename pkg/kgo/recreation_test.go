...
// contract against a real broker: a transaction across a recreation fails
// loudly, wrapping either TRANSACTION_ABORTABLE or, for the commit refusal,
// OPERATION_NOT_ATTEMPTED, aborting recovers, and a bounded number of rounds
// later a transaction commits cleanly into the new incarnation, which
// read-committed consumption then sees exactly.
...
// real broker: an ID we have known for recreationStableIDAge is trusted
// outright, so a paused consumer - no fetches, hence no broker rejection -
// adopts a recreation on a single metadata observation. The pause is the
// discriminator, and it is only airtight where brokers fetch by topic ID
// (3.1+); below that the two-consecutive-updates rule could also swap, so
// we skip. The minute of wall clock overlaps the rest of the suite via
// t.Parallel.
