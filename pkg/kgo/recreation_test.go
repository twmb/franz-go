...
func TestUpdateUncommittedSkipsOldTopicID(t *testing.T) {
	...
	tp.v.Store(&d)
	cl := new(Client)
	cl.sawRecreation.Store(true) // a client only sees differing IDs after a recreation
	g := &groupConsumer{cl: cl, cfg: &cfg{logger: new(nopLogger)}, tps: tps}
...
func TestRecreationEOS(t *testing.T) {
	...
	// This test is about churn, not volume: what it asserts (every record
	// of the final incarnation consumed, each one in the output topic
	// exactly once) does not get truer with more records, and the default
	// testRecordLimit is ETL sized. Cap it so eight recreations against a
	// real broker fit the package timeout; KGO_TEST_RECORDS still lowers it.
	total := min(testRecordLimit, 20000)
	if total < 10 {
...
func TestPriorTopicIDs(t *testing.T) {
	t.Parallel()

	ids := func(p priorTopicIDs) []byte {
		var got []byte
		for _, e := range p {
	...
	now := time.Now()
	var p priorTopicIDs
	if p.any() || p.has([16]byte{1}) {
		t.Fatal("an empty set reports an ID")
	}

	// A flip back re-adds the ID at the front; it does not duplicate.
	p.add([16]byte{1}, now)
	p.add([16]byte{2}, now)
	p.add([16]byte{1}, now)
	if got := ids(p); !bytes.Equal(got, []byte{1, 2}) {
		t.Fatalf("add did not dedupe to the front: %v", got)
	}
	if p.has(noID) {
		t.Fatal("has reports the zero ID")
	}

	// Bounded to the newest.
	for i := byte(3); i <= 6; i++ {
		p.add([16]byte{i}, now)
	}
	if got := ids(p); len(got) != maxPriorTopicIDs || got[0] != 6 || p.has([16]byte{1}) {
		t.Fatalf("add did not bound to the newest %d: %v", maxPriorTopicIDs, got)
	}

	// An ID reported again lives on past the others, and the set is nil
	// again once the last one expires.
	p.refresh([16]byte{4}, now.Add(time.Minute))
	p.dropExpired(now.Add(priorTopicIDExpiry))
	if got := ids(p); !bytes.Equal(got, []byte{4}) {
		t.Fatalf("dropExpired kept %v; want the refreshed 4 alone", got)
	}
	p.dropExpired(now.Add(priorTopicIDExpiry))
	if got := ids(p); !bytes.Equal(got, []byte{4}) {
		t.Fatalf("dropExpired with nothing expired changed the set: %v", got)
	}
	p.dropExpired(now.Add(priorTopicIDExpiry + time.Minute))
	if p != nil || p.any() {
		t.Fatalf("dropExpired left %v; want nil", ids(p))
	}
}
