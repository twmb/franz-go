...
type offsetLoad struct {
	...
	// recreationSeed marks the reset load added when a cursor swaps across
	// topic incarnations. The resolved position also seeds the group's
	// uncommitted entry, lifting the recreation commit fence, and is
...
func (s *consumerSession) handleListOrEpochResults(loaded loadedOffsets) (reloads listOrEpochLoads) {
	...
	for _, load := range loaded.loaded {
		...
		use := func() {
			...
			// A recreation reset resolved, so we seed the group's
			// uncommitted entry with the position and commit it
			// promptly. We dispatch async because we hold
			// listOrEpochMu and the g.mu => listOrEpochMu order is
			// already taken by the fetch-offsets assign path; taking
			// g.mu inline would invert it.
