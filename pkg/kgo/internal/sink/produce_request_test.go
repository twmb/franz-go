...
func TestRecBatchAppendTo(t *testing.T) {
	...
	fixFields := func() {
		...
		rawBatch = ourBatch.appendTo(nil, version, nil, 12, 11, true)
	...
	check := func() {
		...
		gotFull := ourBatch.appendTo(nil, version, compressor, 12, 11, true)
	...
	compressor, _ = compress.NewCompressor(2) // snappy
	defer compressor.Close()
	{
		kbatch.Attributes |= 0x0002 // snappy
		kbatch.Records, _ = compressor.Compress(compressor.GetBuf(), kbatch.Records, 99)
	}
	...
	ourReq.batches.addBatch("topic", 1, &ourBatch)
...
func TestMessageSetAppendTo(t *testing.T) {
	...
	kset0_c.Value, _ = compressor.Compress(compressor.GetBuf(), kset0_raw, 1) // version 0, 1 use message set 0
	...
	kset1_c.Value, _ = compressor.Compress(compressor.GetBuf(), kset1_raw, 2) // version 2 use message set 1
...
func BenchmarkAppendBatch(b *testing.B) {
	...
	ourReq.batches.addBatch("topic 1", 1, &ourBatch)
	ourReq.batches.addBatch("topic 1", 2, &ourBatch)
	ourReq.batches.addBatch("topic 1", 3, &ourBatch)
	ourReq.batches.addBatch("topic 1", 4, &ourBatch)
	ourReq.batches.addBatch("topic 2", 1, &ourBatch)
	ourReq.batches.addBatch("topic 2", 2, &ourBatch)
	...
	for _, pair := range []struct {
		...
		b.Run(pair.name, func(b *testing.B) {
			compressor, _ := compress.NewCompressor(pair.codec)
			if compressor != nil {
				defer compressor.Close()
			}
