...
func (ps pools) each(fn func(Pool)) {
	for _, p := range ps {
		fn(p)
	}
}
...
type PoolRequestBuffer interface {
	GetRequestBuffer(n int) []byte
	PutRequestBuffer([]byte)
}
*/
...
// krecs and recs are *always* non-nil. If the user did not give us pooled
// values, we create our own. The user could only pool one of the three, that's
// fine.
func recordPoolsCtx(pools []Pool, decompressBytes []byte, krecs []kmsg.Record, recs []Record) (*recordPools, context.Context) {
...
func (r *Record) Recycle() {
	...
	*r = Record{} // prevent the Record from hanging onto anything; *kmsg.Record is already cleared during processing
	...
	ps.krecs = ps.krecs[:cap(ps.krecs)]
	...
	// been created!).
	pools(ps.pools).each(func(p Pool) {
