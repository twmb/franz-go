...
func TestProduceRequestNullBatchEncoding(t *testing.T) {
	...
	for _, version := range []int16{8, 9} { // last non-flexible, first flexible
		...
		req.batches.addSeqBatch("t", [16]byte{1}, 0, 0, seqRecBatch{0, &recBatch{}}) // records nil: the null placeholder arm
		req.batches.addSeqBatch("t", [16]byte{1}, 0, 1, seqRecBatch{0, &recBatch{}})
