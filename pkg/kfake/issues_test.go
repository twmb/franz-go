...
// every produce with UNKNOWN_TOPIC_ID. That error is retryable and the
...
// forever. A topic deleted and recreated instead continues under the new ID.
func TestProduceUnknownFailLimitDeletedTopic(t *testing.T) {
	...
	// topic's ID on its per-partition produce buffer.
	...
	// no new topic ever exists to continue under.
	adm := kadm.NewClient(cl)
