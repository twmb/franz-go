...
// what it has always meant there. The default looks back from the last record
// consumed, and there is none, so the lookback falls away and leaves the
// offset's position, the log start: resolved against the clock instead it
// would match nothing in an older log and skip every record still in it. A
// start offset set alone does not reach here, so AtEnd resolves to the log
// start too. NoResetOffset surfaces the error rather than resolving,
...
func TestOutOfRangeNothingConsumed(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			c := newCluster(t, NumBrokers(1), SeedTopics(1, topic))
			// An hour old, so a one minute lookback matches nothing.
...
// where it resumes. The default looks back a minute from the last record
...
func TestOutOfRangeReset(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "lookback rewinds", nconsume: nrecs, apart: 10 * time.Second, want: 3},
		{name: "lookback stays on the last record", nconsume: nrecs, apart: 2 * time.Minute, want: nrecs - 1},
...
// ten minutes and consumes with a five minute lookback. As a start offset the
// client resolves the timestamp when it lists the partition, so consuming
// begins halfway in rather than at the log start. As a reset offset set alone
// the lookback is for a loss, and the start offset it sets is the position
// under it, so consuming begins at the log start.
func TestLookbackStartOffset(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "start offset", opt: kgo.ConsumeStartOffset(kgo.NewOffset().Lookback(5 * time.Minute)), want: 5},
		{name: "reset offset alone", opt: kgo.ConsumeResetOffset(kgo.NewOffset().Lookback(5 * time.Minute)), want: 0},
	} {
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()
			...
			// Record 5 is stamped thirty seconds after the five minute
			// boundary and record 4 thirty seconds before it, so the
			// boundary the client computes a moment later still falls
			// between them.
			produceSpaced(t, c, topic, nrecs, time.Now().Add(-10*time.Minute+30*time.Second), time.Minute)

			cl := newPlainClient(t, c, kgo.ConsumeTopics(topic), test.opt, kgo.FetchMaxWait(100*time.Millisecond))
