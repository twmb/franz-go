...
// A partition whose out of range reset was deferred for classification must
// not spin the source while the classifying metadata is outstanding. With a
// co-partition on the source the response is never all-stripped, so the
// all-stripped backoff does not apply; without pacing, the out of range
// partition stays in the fetch session and every fetch returns immediately
// with the error.
func TestRecreationClassifyPacing(t *testing.T) {
	t.Parallel()

	c := newCluster(t, NumBrokers(1), SeedTopics(1, "t", "u"), MaxVersions(kversion.V2_7_0()))
	cl := newPlainClient(t, c,
		kgo.ConsumeTopics("t", "u"),
		kgo.ConsumeResetOffset(kgo.NewOffset().AtStart()),
		kgo.FetchMaxWait(250*time.Millisecond),
	)
	admin := newPlainClient(t, c)

	produceVals(t, c, "t", 0, "v0", "v1", "v2", "v3", "v4")
	collectVals(t, cl, "v0", "v1", "v2", "v3", "v4")

	cl.PauseFetchTopics("t")
	time.Sleep(300 * time.Millisecond)
	recreateTopic(t, admin, "t", 1)
	produceVals(t, c, "t", 0, "n0")

	// Hold every metadata response so the classification cannot land,
	// and count fetches meanwhile.
	...
	n := fetches.Load()
	close(release)
	t.Logf("fetches while classification was held: %d", n)
	if n > 10 {
		t.Errorf("saw %d fetches while classification was held; want the refetch paced", n)
	}
