...
func TestSchemaRegistryAPI(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			fn:       func() (any, error) { return c.Subjects(InContext(ctx, ".mycontext")) },
			expected: `["foo"]`,
		},
		{
			name:     "get context schema by ID",
			fn:       func() (any, error) { return c.SchemaByID(InContext(ctx, ".mycontext"), 1) },
			expected: `{"schema":"{\"name\":\"foo\", \"type\": \"record\", \"fields\":[{\"name\":\"str\", \"type\": \"string\"}]}"}`,
		},
		{
			name:     "get context subjects by ID",
			fn:       func() (any, error) { return c.SubjectsByID(InContext(ctx, ".mycontext"), 1) },
			expected: `["foo"]`,
		},
		{
			name:     "get context schema versions by ID",
			fn:       func() (any, error) { return c.SchemaVersionsByID(InContext(ctx, ".mycontext"), 1) },
			expected: `[{"subject":"foo","version":1}]`,
		},
	}
...
func TestCreateSchemaWithContext(t *testing.T) {
	...
	t.Run("InContext", func(t *testing.T) {
		...
		ss, err := c.CreateSchema(InContext(ctx, ".myctx"), "foo", dummySchema)
		if err != nil {
			t.Fatalf("CreateSchema with InContext failed: %s", err)
		}
